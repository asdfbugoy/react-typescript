...
export class InnerWrapper extends React.Component {
	...
	render() {
		...
		const items = data.map((el) => React.cloneElement(el, { selected: isActive(el.key, selected) }));
...
export class ScrollMenu extends React.Component {
	...
	shouldComponentUpdate(nextProps, nextState) {
		...
		const { translate: translateNew, dragging: draggingNew } = nextState;

		const { translate: translateProps, selected: selectedProps } = this.props;
		const { translate: translatePropsNew, selected: selectedPropsNew } = nextProps;

		const translatePropsDiff = notUndefOrNull(translatePropsNew) && translateProps !== translatePropsNew;
		...
		return newMenuItems || translate !== translateNew || dragging !== draggingNew || propsDiff;
	}
	...
	setInitial = () => {
		...
		if (translate !== translateNew && typeof translateNew === 'number') {
			this.setState({ translate: translateNew }, () => this.onUpdate({ translate: translateNew }));
		}
	};

	getMenuItems = (dataLength) => {
		return Object.entries(this.ref).filter((el) => el[0].includes('menuitem')).slice(0, dataLength).filter(Boolean);
	};

	getItemsWidth = ({ items = this.menuItems }) => {
		const data = (items && items.items) || items;
		// TODO maybe while cycle faster, need test
		return data.map((el) => el[1]).filter(Boolean).reduce((acc, el) => (acc += getClientRect(el).width), 0);
	};
	...
	getOffsetToItem = ({ itemId, menuItems = this.menuItems, translate = this.state.translate }) => {
	...
	getScrollRightOffset = (visibleItems, items) => {
		...
		const nextItem = this.getNextItem(
			visibleItems && visibleItems.slice(-1)[0] ? visibleItems.slice(-1)[0][0] : items.slice(-1)[0][0]
		);
		...
		const newOffset = alignCenter ? offsetToItemOnStart - centerOffset() : offsetToItemOnStart;
		return newOffset;
	};
	...
	getScrollLeftOffset = (visibleItems, items) => {
		...
		const prevItem = this.getPrevItem(visibleItems && visibleItems[0] ? visibleItems[0][0] : items[0][0]);
		...
		const newOffset = alignCenter ? offsetToItemOnEnd + centerOffset() : offsetToItemOnEnd;
		return newOffset;
	};
	...
	handleDragStop = (e) => {
		const { allItemsWidth, menuWidth, firstPageOffset, lastPageOffset, startDragTranslate } = this;
		let { dragging, xPoint = this.getPoint(e), translate } = this.state;
